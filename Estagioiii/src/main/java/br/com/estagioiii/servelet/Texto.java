    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!("").equalsIgnoreCase(request.getParameter("tende"))) {
            ...
            for (int i = 0; i <= Integer.parseInt(request.getParameter("tende")); i++) {
                if (request.getParameter("textf" + i) != null) {
                    QuestionarioModel questionarioModel = new QuestionarioModel();
                    questionarioModel.setPergunta(request.getParameter("textf" + i));
                    questionarioModel.setTipoQuestionarioModel(TipoQuestionarioDao.tipoQuestionario(1));
